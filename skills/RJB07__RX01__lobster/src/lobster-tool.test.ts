...
import type { RX01PluginApi, RX01PluginToolContext } from "rx01/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<RX01PluginApi> = {}): RX01PluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<RX01PluginToolContext> = {}): RX01PluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rx01-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: RX01PluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
