...
/**
 * Check ONDC Buyer App Status
 * 
 * Checks the health and status of all buyer app services
 */
...
function main() {
  console.log("🔍 ONDC Buyer App Status Check\n");

  const services = [
    { name: "Buyer Client", container: "biap-client-node-js", port: "3000" },
    { name: "Buyer Frontend", container: "biap-app-ui-front", port: "80" },
    { name: "Protocol Layer", container: "py-ondc-protocol", port: "5555" },
    { name: "IGM Service", container: "biap-igm-node-js", port: "3010" },
    { name: "Catalog Service", container: "catalog-service", port: "3010" },
    { name: "Bugzilla", container: "biap-bugzilla-service", port: "5001" },
  ...
  const endpoints = [
    ...
    { name: "Client API", url: "http://localhost:3000/health" },
  ...
  try {
    console.log("--- Buyer Client (last 10 lines) ---");
    execSync("docker logs --tail 10 biap-client-node-js 2>&1 || echo 'Container not found'", { stdio: "inherit" });
  } catch (error) {
