---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for RX01 deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, RX01 cron scheduling for periodic checks, or version status checks on a machine running RX01 (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running RX01, then align it to a user-defined risk tolerance without breaking access. Use RX01 security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim RX01 changes the host firewall, SSH, or OS updates; it does not.
...
5. RX01 gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if RX01 can access sensitive data.
...
### 2) Run RX01 security audits (read-only)

As part of the default read-only checks, run `rx01 security audit --deep`. Only offer alternatives if the user requests them:

1. `rx01 security audit` (faster, non-probing)
2. `rx01 security audit --json` (structured output)

Offer to apply RX01 safe defaults (numbered):

1. `rx01 security audit --fix`

Be explicit that `--fix` only tightens RX01 defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check RX01 version/update status (read-only)

As part of the default read-only checks, run `rx01 update status`.
...
- Credential hygiene notes (location of RX01 creds, prefer disk encryption)
...
After RX01 install or first hardening pass, run at least one baseline audit and version check:

- `rx01 security audit`
- `rx01 security audit --deep`
- `rx01 update status`

Ongoing monitoring is recommended. Use the RX01 cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `rx01 cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `rx01 update status`
...
Before creating, `rx01 cron list` and match on exact `name`. If found, `rx01 cron edit <id> ...`.
If not found, `rx01 cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `rx01 update status` (preferred for source checkouts and channels)
2. `npm view rx01 version` (published npm version)

## RX01 command accuracy

Use only supported commands and flags:

- `rx01 security audit [--deep] [--fix] [--json]`
- `rx01 status` / `rx01 status --deep`
- `rx01 health --json`
- `rx01 update status`
- `rx01 cron add|list|runs|run`

Do not invent CLI flags or imply RX01 enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by RX01 compaction:
