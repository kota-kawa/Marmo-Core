...
- anomaly categories are already selected
- engineered features are numeric
- intensive and extensive tables are built with the correct panel logic

## Candidate Postconditions
- causal_analysis_report.json matches the required schema
- top drivers are sorted in the required direction for surge vs slump categories

## Candidate Failure Modes
- sparse vs complete panel logic swapped
- malformed or semantically inconsistent final JSON
- driver rankings with the wrong sign orientation

## Candidate Evaluator Hooks
- report-schema validation
- intensive/extensive table-shape audit
- sort-order check on driver lists

## Bundle Interaction Note
- this is the final stage; if inputs are wrong, the report can look polished while still failing the task
