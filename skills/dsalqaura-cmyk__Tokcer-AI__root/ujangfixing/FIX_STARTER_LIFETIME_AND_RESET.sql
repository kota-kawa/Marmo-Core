-- ==============================================================================
-- 🏮 TOKCER AI: PERBAIKAN PERMANEN STARTER PLAN (LIFETIME & AUTO-RESET)
-- ==============================================================================
-- Masalah:
-- 1. Akun Starter (Gratis) dianggap memiliki masa aktif bulanan (expires_at diset 30 hari).
--    Akibatnya, setelah 30 hari robot harian mengunci akun mereka menjadi 'expired'
--    sehingga mereka harus registrasi/aktivasi ulang terus-menerus.
-- 2. Akun Starter tidak memiliki mekanisme reset otomatis bulanan untuk jatah 50 token.
--
-- Solusi Permanen:
-- 1. Mengubah rpc_activate_account agar jika p_plan = 'starter', expires_at diset NULL (Lifetime).
-- 2. Mengubah rpc_daily_subscription_check agar:
--    - Meng-exclude plan 'starter' dari penguncian akun kedaluwarsa.
--    - Memasukkan plan 'starter' ke sistem reset token bulanan otomatis (50 token) setiap tanggal join.
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- FASE 1: PERBAIKAN MESIN AKTIVASI (rpc_activate_account)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- JIKA STARTER, SET EXPIRES_AT = NULL (LIFETIME / SELAMANYA)
    v_expires_at := CASE 
...
-- ------------------------------------------------------------------------------
-- FASE 2: PERBAIKAN MESIN CRON HARIAN (rpc_daily_subscription_check)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- ==============================================================================
    -- A. RESET TOKEN UNTUK USER TAHUNAN & USER STARTER (Setiap Bulan pada Tanggal Join)
    -- ==============================================================================
    FOR v_client IN 
        ...
        WHERE c.status = 'active' 
          AND (c.billing_cycle = 'Yearly' OR c.plan = 'starter')
          AND EXTRACT(DAY FROM c.created_at) = EXTRACT(DAY FROM NOW())
    LOOP
        v_base_tokens := CASE 
            WHEN v_client.plan = 'starter' THEN 50
    ...
    -- ========================================================
    -- B. EMAIL H-3 JATUH TEMPO UNTUK USER BULANAN (PAID BULANAN)
    -- ========================================================
    FOR v_client IN 
        SELECT email, shop_name, plan, expires_at 
        FROM public.clients 
        WHERE status = 'active' AND billing_cycle = 'Monthly' AND plan <> 'starter'
    ...
    -- ========================================================
    -- C. KUNCI AKUN BULANAN YANG SUDAH LEWAT MASA AKTIF (EXCLUDE STARTER)
    -- ========================================================
    FOR v_client IN 
        ...
        WHERE status = 'active' 
          ...
          AND plan <> 'starter' -- JANGAN PERNAH KUNCI AKUN STARTER (LIFETIME)
          AND expires_at < NOW()
    LOOP
