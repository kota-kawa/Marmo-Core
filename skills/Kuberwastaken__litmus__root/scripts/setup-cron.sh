...
# --- Archive interval: convert days → milliseconds ---
ARCHIVE_MS=$(( ARCHIVE_DAYS * 86400000 ))
...
echo "Archive:        every ${ARCHIVE_DAYS} days (cleanup + checkpoint)"
echo ""
echo "Ask your OpenClaw agent to run these 7 cron tool calls:"
echo ""
...
============================================================
CRON 7: Periodic Archive (every ${ARCHIVE_DAYS} days — cleanup + checkpoint)
============================================================
cron action:"add" job:{
  "name": "litmus-archive",
  "enabled": true,
  "schedule": { "kind": "every", "everyMs": ${ARCHIVE_MS} },
  "sessionTarget": "main",
  "wakeMode": "now",
  "payload": {
    "kind": "agentTurn",
    "message": "Run the Litmus periodic archive. Full instructions in $REFERENCES_DIR/archive.md — read that file first. Archive old attempts with ARCHIVE_DAYS=${ARCHIVE_DAYS}. Run bash {baseDir}/scripts/cleanup.sh, then write a ${ARCHIVE_DAYS}-day research checkpoint to $SHARED_DIR/notes/synthesis/. Promote any long-pending skills eligible for relaxed promotion."
  },
  "delivery": { "mode": "none" }
}
EOF
...
echo "  Every ${ARCHIVE_DAYS}d  Archive   — cleanup old attempts + research checkpoint"
