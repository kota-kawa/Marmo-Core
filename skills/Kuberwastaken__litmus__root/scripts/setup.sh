...
# --- Disk space check (need ~2 GB for harness + data + repo) ---
AVAIL_KB=$(df -k "$HOME" | awk 'NR==2 {print $4}')
AVAIL_GB=$(awk "BEGIN {printf \"%.1f\", $AVAIL_KB/1024/1024}")
if [ "$AVAIL_KB" -lt 2097152 ]; then   # < 2 GB
  echo "ERROR: Insufficient disk space. Need at least 2 GB, found ${AVAIL_GB} GB available."
  echo "Free up space and re-run setup."
  exit 1
elif [ "$AVAIL_KB" -lt 5242880 ]; then  # < 5 GB
  echo "Warning: Low disk space (${AVAIL_GB} GB available). Recommend at least 5 GB for comfortable operation."
  echo "Continuing — but consider running scripts/cleanup.sh regularly."
else
