...
### 11. Submit to the Skills Pipeline (on any improvement)

Skills go through a **two-stage pipeline** before becoming canon:

1. **You write to `pending/`** — unvalidated, based on your single improvement
2. **The Synthesizer promotes to `skills/`** — only after ≥3 corroborating experiments
   from ≥2 different agents confirm the technique generalises

Write to `SHARED_DIR/skills/pending/` — NOT directly to `SHARED_DIR/skills/`:

```bash
SKILL_NAME="[category]-[slug]"  # e.g. "arch-depth10-baseline" or "opt-matrix-lr-0.06"
cat > "SHARED_DIR/skills/pending/$SKILL_NAME.md" << 'EOF'
---
name: [skill name]
author: AGENT_ID
...
[exact code change]
```
**Evidence**: <hash> by AGENT_ID — val_bpb improved by [delta] ([before] → [after])
...
**Reading skills**: Before forming a hypothesis, read BOTH canon and pending skills:
```bash
# Canon skills (fully validated, high confidence)
for f in SHARED_DIR/skills/*.md; do [ "$f" != "SHARED_DIR/skills/INDEX.md" ] && cat "$f"; done
# Pending skills (1 confirmation — treat as promising leads, not proven techniques)
for f in SHARED_DIR/skills/pending/*.md; do cat "$f"; done
```

The Synthesizer runs at 04:00 and promotes pending skills that have accumulated enough
independent evidence. Until then, treat pending skills as high-priority hypotheses to
corroborate — not as validated techniques to blindly stack.
