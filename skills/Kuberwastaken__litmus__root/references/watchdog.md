...
# Check disk usage
USED_PCT=$(df "$BASE_DIR" | awk 'NR==2 {gsub("%",""); print $5}')
AVAIL=$(df -h "$BASE_DIR" | awk 'NR==2 {print $4}')
ATTEMPTS_COUNT=$(ls "$BASE_DIR/shared/attempts"/*.json 2>/dev/null | wc -l | tr -d ' ')

echo "Disk: ${USED_PCT}% used, ${AVAIL} available, ${ATTEMPTS_COUNT} attempt files"
```

**If disk usage ≥ 90%** — warn:
```bash
openclaw system event --text "Warning: Litmus disk at ${USED_PCT}% — ${ATTEMPTS_COUNT} attempt files. Run: bash {baseDir}/scripts/cleanup.sh to free space." --mode now
```

**If disk usage ≥ 95%** — trigger cleanup automatically:
```bash
bash {baseDir}/scripts/cleanup.sh --keep-days 3 --keep-top-pct 40
openclaw system event --text "Litmus watchdog: disk at ${USED_PCT}% — auto-cleanup triggered. ${ATTEMPTS_COUNT} attempts archived." --mode now
```

Log to `SHARED_DIR/watchdog-log.md`:
```
[timestamp] DISK_CHECK: ${USED_PCT}% used, ${AVAIL} available, ${ATTEMPTS_COUNT} attempts
```
