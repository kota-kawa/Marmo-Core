---
name: node-connect
description: Diagnose RX01 node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify RX01 is advertising that route, then fix pairing/auth.
...
- whether `rx01 devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `rx01 qr --json`. It uses the same setup-code payload Android scans.

```bash
rx01 config get gateway.mode
rx01 config get gateway.bind
rx01 config get gateway.tailscale.mode
rx01 config get gateway.remote.url
rx01 config get gateway.auth.mode
rx01 config get gateway.auth.allowTailscale
rx01 config get plugins.entries.device-pair.config.publicUrl
rx01 qr --json
rx01 devices list
rx01 nodes status
```

If this RX01 instance is pointed at a remote gateway, also run:
...
If `rx01 qr --json` says `Gateway is only bound to loopback`:
...
- `rx01 devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run rx01 qr again, rescan, then approve the pending device pairing.`
